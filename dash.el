...
by `dash--match-kv'."
  (let ((normalized (list (car match-form)))
        (skip nil))
    (-each (apply '-zip (-pad '&--dash--fill-placeholder (cdr match-form) (cddr match-form)))
      (lambda (pair)
        (let ((current (car pair))
              (next (cdr pair)))
          (if skip
              (setq skip nil)
            (if (or (eq '&--dash--fill-placeholder next)
                ...
                (progn
                  (cond
                   ...
                   (t (error "-let: found key `%s' in kv destructuring but its match-form `%s' is invalid and can not be derived from the key" current next)))
                  (setq skip nil))
              (push current normalized)
